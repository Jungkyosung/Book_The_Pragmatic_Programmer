...
고객용 클래스인 AccountForCustomer는 고객이 사용하는 API에 적합한 검증을 추가로 포함하고 있고,  
관리자용 클래스인 AccountForAdmin은 아마 제한이 더 느슨할 관리자용 검증을 추가로 포함하고 있다.  

이제 AccountForCustomer와 AccountForAdmin의 인스턴스를 이리저리 넘길 때 자동으로 정확한 검증을 수행하리라 보장할 수 있다.  

"믹스인으로 기능을 공유하라."

**"상속이 답인 경우는 드물다."**
잘 판단해서 상속의 대안을 먼저 사용해라.  


#### Topic32. 설정

"모든 물건은 제자리에 두고, 일은 모두 때를 정해서 하라." - 벤저민 플랭클린 - 
...
설정정보를 보통은 정적으로 관리한다.  
예를 들면, YAML이나 JSON파일로 관리하거나, 데이터베이스 테이블에 관리한다.  
이런 정보를 애플리케이션 실행시 읽어와서 사용한다.  
이 자료들을 전역에서 접근할 수 있도록 하고, 코드의 어느 부분에서든 설정 정보에 쉽게 접근할 수 있도록 한다.  

**서비스형 설정**

서비스 API 뒤에서 관리하는 서비스형 설정이 있다.  

서비스형 설정의 장점
- 여러 애플리케이션이 설정 정보를 공유할 수 있다. 인증과 접근 제어를 붙여서 애플리케이션마다 보이는 정보가 다르게 만들 수 있음.
- 여러 인스턴스에 걸쳐서 전체 설정을 한 번에 바꿀 수 있음.
- 설정 데이터를 전용 UI로 관리할 수 있음.
- 설정 데이터를 동적으로 계속 바꿀 수 있음.

설정 데이터를 동적으로 바꿀 수 있기 때문에 고가용성 애플리케이션을 만들 때 매우 중요하다.  
설정값 하나를 바꾸기 위해 애플리케이션을 멈추고 다시 시작해야 하는 상황을 피할 수 있다.  

**너무 과하게 모든 것(변수)을 서비스형 설정으로 다루진 말라**
